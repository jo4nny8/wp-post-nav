...
5. For integration testing, install the WordPress test suite and set `WP_TESTS_DIR` to its location. The PHPUnit bootstrap must load WordPress before integration tests can exercise queries, templates, shortcodes, and database state.
...
The exact integration bootstrap command depends on the local WordPress test installation and must load WordPress before the integration suite is enabled.
