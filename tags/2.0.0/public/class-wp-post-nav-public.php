...
class wp_post_nav_Public {
    ...
    elseif (array_key_exists('wp_post_nav_post_types', $settings)) {
		...
		if (class_exists('WPSEO_Primary_Term') && array_key_exists('wp_post_nav_exclude_primary') || function_exists( 'the_seo_framework' ) && array_key_exists('wp_post_nav_exclude_primary', $settings)) {
			$exclude_primary = 'yes';
		}
		else {
			$exclude_primary = 'no';
